// Code generated by go-swagger; DO NOT EDIT.

package payment_service

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	models "github.com/siriscac/microservices-demo/fontend/apiclient/models"
)

// ChargeReader is a Reader for the Charge structure.
type ChargeReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *ChargeReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewChargeOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("unknown error", response, response.Code())
	}
}

// NewChargeOK creates a ChargeOK with default headers values
func NewChargeOK() *ChargeOK {
	return &ChargeOK{}
}

/*ChargeOK handles this case with default header values.

A successful response.
*/
type ChargeOK struct {
	Payload *models.HipstershopChargeResponse
}

func (o *ChargeOK) Error() string {
	return fmt.Sprintf("[POST /payments][%d] chargeOK  %+v", 200, o.Payload)
}

func (o *ChargeOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.HipstershopChargeResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
